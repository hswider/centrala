import { getAllegroTokens, saveAllegroTokens } from './db';
...
const getCredentials = () => ({
  ...
  redirectUri: process.env.ALLEGRO_REDIRECT_URI || 'https://centrala-poom.vercel.app/api/allegro/callback'
});
...
export async function exchangeCodeForTokens(code) {
  ...
  await saveAllegroTokens(data.access_token, data.refresh_token, expiresAt);
...
export async function getValidAccessToken() {
  let tokens = await getAllegroTokens();

  if (!tokens?.access_token) {
    throw new Error('Not authenticated with Allegro');
  }

...
import {
  ...
  isAuthenticated,
  parseMessage
} from '../../../../../lib/gmail-poomfurniture';
import {
  initDatabase,
  getGmailPoomfurnitureThread,
  saveGmailPoomfurnitureMessage,
  saveGmailPoomfurnitureThread,
  markGmailPoomfurnitureThreadAsRead,
  getGmailPoomfurnitureTokens
} from '../../../../../lib/db';
...
export async function GET(request, { params }) {
  try {
    ...
    if (refresh) {
      try {
        ...
        if (gmailThread && gmailThread.messages) {
          ...
          // Save thread
          await saveGmailPoomfurnitureThread({
            id: threadId,
          ...
          for (const msg of gmailThread.messages) {
            ...
            await saveGmailPoomfurnitureMessage({
              id: parsed.id,
        ...
        console.error('Failed to refresh Gmail POOMFURNITURE messages:', refreshError.message);
      }
    }

    // Get thread with messages from database
    const data = await getGmailPoomfurnitureThread(threadId);
    ...
    console.error('Get Gmail POOMFURNITURE thread error:', error);
    return NextResponse.json(
...
export async function POST(request, { params }) {
  try {
    ...
    console.error('Send Gmail POOMFURNITURE message error:', error);
    return NextResponse.json(
      { success: false, error: error.message },
      { status: 500 }
    );
...
export async function PUT(request, { params }) {
  try {
    ...
    try {
      ...
      console.error('Failed to mark as read in Gmail POOMFURNITURE:', e.message);
    }

    // Mark as read in database
    await markGmailPoomfurnitureThreadAsRead(threadId);

...
import {
  ...
  isAuthenticated,
  parseMessage
} from '../../../../../lib/gmail-allepoduszki';
import {
  initDatabase,
  getGmailAllepoduszkiThread,
  saveGmailAllepoduszkiMessage,
  saveGmailAllepoduszkiThread,
  markGmailAllepoduszkiThreadAsRead,
  getGmailAllepoduszkiTokens
} from '../../../../../lib/db';
...
export async function GET(request, { params }) {
  try {
    ...
    if (refresh) {
      try {
        ...
        if (gmailThread && gmailThread.messages) {
          ...
          // Save thread
          await saveGmailAllepoduszkiThread({
            id: threadId,
          ...
          for (const msg of gmailThread.messages) {
            ...
            await saveGmailAllepoduszkiMessage({
              id: parsed.id,
        ...
        console.error('Failed to refresh Gmail ALLEPODUSZKI messages:', refreshError.message);
      }
    }

    // Get thread with messages from database
    const data = await getGmailAllepoduszkiThread(threadId);
    ...
    console.error('Get Gmail ALLEPODUSZKI thread error:', error);
    return NextResponse.json(
...
export async function POST(request, { params }) {
  try {
    ...
    console.error('Send Gmail ALLEPODUSZKI message error:', error);
    return NextResponse.json(
      { success: false, error: error.message },
      { status: 500 }
    );
...
export async function PUT(request, { params }) {
  try {
    ...
    try {
      ...
      console.error('Failed to mark as read in Gmail ALLEPODUSZKI:', e.message);
    }

    // Mark as read in database
    await markGmailAllepoduszkiThreadAsRead(threadId);
